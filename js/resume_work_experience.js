mui.plusReady(function(){
	...
	mui(document).on('tap','.add_edu',function(){
		openWindow('resume_work_experience_edit.html',{
			type : 'add'
		});
	});
	...
	mui(document).on('tap','.mui-btn-red',function(){
		...
		plus.nativeUI.confirm('确定删除？',function(e){
			if(e.index == 0){
				...
				mui.post(getLocation('Resume/delUserWorkExperience.html'),option,function(result){
	...
	function loadData(){
		...
		loading(0);
		mui.post(getLocation('Resume/getUserWorkExperience.html'),option,function(result){
			loading(1);
